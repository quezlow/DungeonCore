#!/usr/bin/env python3
"""
deliver_den_sims.py -- restores a destroyed measurement tool and adds the one
the kobold digger arc needs.

WHAT THIS FIXES, AND WHY IT IS NOT A TIDY-UP.

Commit 78f52cd6 ("Den cavity growth") wrote the cavity-growth simulation to
Tools/sim_den_tunnels.py as well as to its own name, destroying the DEN TUNNEL
substrate sim in the process. Two consequences, both silent:

  1. Tools/sim_den_cavity.py imports twelve names from sim_den_tunnels and has
     been dead on import ever since -- ImportError on CHAMBER_SPACING. That is
     the STANDING REGRESSION SIM canon 42 says is re-swept whenever a band
     moves, so the check has been unavailable for as long as the fault has.
  2. Docs/DESIGN_CANON.md cites Tools/sim_den_tunnels.py as the source of the
     measured tunnel table -- 2.30 chamber links and 0.70 dead ends on floor
     index 1, 3.38 and 0.62 on floor index 2. The citation pointed at a file
     that no longer contained the measurement.

The restore is the file as it stood at 600668f2 ("den substrate"), verified to
resolve all twelve imports and to reproduce canon's quoted figures exactly.

WHAT THIS ADDS. Tools/sim_den_digger.py, the measurement the digger arc rests
on. It found that a buried remains CANNOT EXIST inside 0.55 of the floor radius
-- GetBuriedSites accepts only Stone or Granite and ComputeRadialBand is
Chebyshev -- so only 12.3 per cent of seeds carry a remains inside the den's
15-65 per cent band, and the beeline dig the arc was specced around would have
had no target on seven seeds in eight.

CANON. One paragraph, recording the clobber beside the citation it broke, so a
later reader wondering why the file's history has a gap finds the answer next
to the numbers it produced.

Idempotent: a second run detects the applied state and aborts without writing.
"""
...
TUNNELS = 'Tools/sim_den_tunnels.py'
DIGGER = 'Tools/sim_den_digger.py'

CANON_ANCHOR = (
    "`TESTING/Commands.cs` (the headless report),\n"
...
def read(rel):
    """Reads a repo file, normalising line endings and remembering which they
    were. The checkout is CRLF and the repository is LF, so an anchor holding a
    bare newline matches nothing against the working tree and fails looking
    exactly like a stale anchor."""
...
def staged(rel, text, crlf, bom):
    out = text.replace('\n', '\r\n') if crlf else text
    data = out.encode('utf-8')
    if bom:
        data = b'\xef\xbb\xbf' + data
    return os.path.join(REPO, rel), data


def ascii_only(label, text):
    for i, line in enumerate(text.split('\n'), 1):
        for ch in line:
            if ord(ch) > 126 or (ord(ch) < 32 and ch != '\t'):
                die('non-ASCII in inserted %s at line %d: %r' % (label, i, ch))


def balanced(label, text):
    pairs = {'(': ')', '[': ']', '{': '}'}
    ...
    for ch in text:
        if ch in pairs:
            ...
            if not stack or stack[-1] != closers[ch]:
                die('bracket imbalance in ' + label)
            stack.pop()
    if stack:
        die('unclosed bracket in ' + label)


def main():
    if not os.path.isdir(os.path.join(REPO, 'Assets', 'Scripts')):
        die('does not look like the DungeonCore repo: ' + REPO)

    canon, canon_crlf, canon_bom = read(CANON)
    tunnels_now, t_crlf, t_bom = read(TUNNELS)

    # -- idempotency ---------------------------------------------------
    already_canon = 'The tunnel sim was destroyed by the cavity-growth commit' in canon
    already_tunnels = 'Headless simulation of the DEN TUNNEL substrate' in tunnels_now
    already_digger = os.path.isfile(os.path.join(REPO, DIGGER))
    if already_canon and already_tunnels and already_digger:
        print('Already applied -- nothing to do.')
        return
    if already_canon or (already_tunnels and already_digger):
        die('PARTIALLY applied (canon=%s tunnels=%s digger=%s). '
            'Reset the tree before re-running.'
            % (already_canon, already_tunnels, already_digger))

    # -- anchor assertions, before a single byte is written ------------
    n = canon.count(CANON_ANCHOR)
    if n != 1:
        die('canon anchor matched %d times, expected exactly 1' % n)

    # -- validate the payload ------------------------------------------
    for label, body in (('sim_den_tunnels.py', TUNNELS_BODY),
                        ('sim_den_digger.py', DIGGER_BODY)):
        ascii_only(label, body)
        balanced(label, body)
    ascii_only('canon insert', CANON_INSERT)

    tmp = tempfile.mkdtemp(prefix='densims_')
    try:
        for label, body in (('sim_den_tunnels.py', TUNNELS_BODY),
                            ('sim_den_digger.py', DIGGER_BODY)):
            probe = os.path.join(tmp, label)
            with open(probe, 'w', newline='\n') as fh:
                fh.write(body)
            try:
                py_compile.compile(probe, doraise=True)
            except py_compile.PyCompileError as exc:
                die('py_compile failed on %s: %s' % (label, exc))
    finally:
        import shutil
    ...
    new_canon = canon.replace(CANON_ANCHOR, CANON_ANCHOR + CANON_INSERT, 1)
    if new_canon == canon:
        die('canon replacement produced no change')
    writes.append(staged(CANON, new_canon, canon_crlf, canon_bom))

    # The two sims are whole-file writes. They take the CHECKOUT's line endings
    # rather than the repository's, matching every other file in the tree.
    writes.append(staged(TUNNELS, TUNNELS_BODY, t_crlf, t_bom))
    writes.append(staged(DIGGER, DIGGER_BODY, t_crlf, False))

    # -- write, then speak. Never interleaved: a BrokenPipeError partway
    # through a printing loop would leave a half-applied tree.
    ...
    print('Applied to ' + REPO)
    print('  restored  ' + TUNNELS + '   (%d bytes)' % len(TUNNELS_BODY))
    print('  added     ' + DIGGER + '    (%d bytes)' % len(DIGGER_BODY))
    print('  canon     ' + CANON + '  (+1 paragraph, entry 42 key files)')
    print('')
    print('Verify:  cd Tools && python3 -c "import sim_den_cavity"')
    print('Run:     cd Tools && python3 sim_den_digger.py 400')
